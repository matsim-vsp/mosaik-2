...
public class NumericSmoothingRadiusEstimateTest {
	...
	public void testWeightWithPlot() {
		...
		try (var writer = Files.newBufferedWriter(Paths.get("C:\\Users\\Janekdererste\\Desktop\\smoothing-estimates\\weight-results.csv"));
			 var printer = new CSVPrinter(writer, Utils.createWriteFormat("x", "value"))) {
	...
	public void testEstimateR() {
		...
		final var cellSize = 20;
		final var normalizationFactor = cellSize / (Math.PI * R * R);

		// first calculate xj with f - this is the forward equation
		var xj = NumericSmoothingRadiusEstimate.calculateWeight(from, to, receiverPoint, le, R) * normalizationFactor * E;

		// now, use xj in estimate R. This should yield the same R which we have used for the forward pass
		var estimatedR = NumericSmoothingRadiusEstimate.estimateR(emissions, receiverPoint, xj, R - 5, cellSize);
	...
	public void testEstimateRWithBisect() {
		...
		try {
			var estimatedR = NumericSmoothingRadiusEstimate.estimateRWithBisect(emissions, receiverPoint, xj, cellSize);
			assertEquals(R, estimatedR, 10E-5);
		} catch (Exception e) {
	...
	public void testEstimateRWithBisect2Links() {
		...
		try {
			var estimatedR = NumericSmoothingRadiusEstimate.estimateRWithBisect(emissions, receiverPoint, xj, 10);
			assertEquals(R, estimatedR, 10E-5);
		} catch (Exception e) {
			if (xj != 0) throw new RuntimeException(e);
			// otherwise, this exception is expected
	...
	public void testWithPlot() {
		...
		try (var writer = Files.newBufferedWriter(Paths.get("C:\\Users\\Janekdererste\\Desktop\\smoothing-estimates\\F-results-" + R + ".csv"));
			 var printer = new CSVPrinter(writer, Utils.createWriteFormat("R", "value"))) {
