...
public class ReactiveEventsManagerTest {
    ...
    public void testEventHandlerCanProduceAdditionalEventLateInSimStep() {
        final ReactiveEventsManager events = new ReactiveEventsManager(8);
    ...
    public void ensureOrder() {
        ...
        var manager = new ReactiveEventsManager(1);
    ...
    public void testOrderOfEventsIsKept() {
        ...
        var manager = new ReactiveEventsManager(3);
        var events = List.of(
    ...
    private static class SlowHandler implements BasicEventHandler {
        ...
        public void handleEvent(Event event) {
            ...
            if (event.getEventType().equals(eventType)) {
                log.info("SlowHandler before sleep: " + event.getEventType() + ", " + event.getTime());
                try {
                    Thread.sleep(1000); // long computation
                } catch (InterruptedException e) {
                    throw new RuntimeException(e);
                }
                log.info("SlowHandler after sleep: " + event.getEventType() + ", " + event.getTime());
    ...
    private static class CounterHandler implements BasicEventHandler {
        ...
        public void handleEvent(Event event) {
            if (event.getEventType().equals(eventType)) {
                log.info("Counter handler " + event.getEventType() + ", " + event.getTime() + " count is: " + counter);
                counter++;
            }
        }
    }
    ...
    @Test
    public void getUsedToFramework() throws InterruptedException {

        var manager = new ReactiveEventsManager(6);
        manager.addHandler((BasicEventHandler) event -> {
            log.info("Slow Handler received: " + event.getEventType() + ": " + event.getTime());
            try {
                Thread.sleep(1000);
                log.info("Slow Handler finished: " + event.getEventType() + ": " + event.getTime());
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        });
        manager.addHandler((BasicEventHandler)event -> {
            log.info("Fast Handler received: " + event.getEventType() + ": " + event.getTime());
        });

        manager.initProcessing();
        manager.processEvent(new Event(1) {
            @Override
            public String getEventType() {
                return "first";
            }
        });
        manager.processEvent(new Event(2) {
            @Override
            public String getEventType() {
                return "second";
            }
